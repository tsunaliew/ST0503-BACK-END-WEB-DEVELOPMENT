  ...
  showFriends: function (userID, callback) {
    ...
    dbConn.connect(function (err) {
      if (err) {
        ...
        const showFriendsQuery = "SELECT u.* FROM user u INNER JOIN friendship f ON (u.id = f.fk_friend_one_id OR u.id = f.fk_friend_two_id) WHERE (f.fk_friend_one_id = ? OR f.fk_friend_two_id = ?) AND u.id <> ?";
